...
class PaymentServiceTest {
	...
	void confirmPayment_Success() {
		...
		when(internalOrderClient.clearCartItems()).thenReturn(new ApiResponse<>(true, "200", "성공", "장바구니 비우기 성공"));
		doReturn("success:{\"status\":\"DONE\"}").when(paymentService).callTossConfirmApi(any(PaymentConfirmRequest.class), any(Long.class));

		// When
		String result = paymentService.confirmPayment(confirmRequest, authentication);
		...
		verify(internalOrderClient).clearCartItems();
	}
	...
	void confirmPayment_ApiCallFailed() {
		...
		// When & Then
		assertThatThrownBy(() -> paymentService.confirmPayment(confirmRequest, authentication))
		...
		verify(paymentRepository).save(any(Payment.class));
		verify(paymentEtcRepository).save(any(PaymentEtc.class));
		verify(internalOrderClient, never()).clearCartItems();
	}
	...
	void confirmPayment_AmountMismatch() {
		...
		// When & Then
		assertThatThrownBy(() -> paymentService.confirmPayment(wrongAmountRequest, authentication))
			.isInstanceOf(GeneralException.class)
			.satisfies(ex -> {
				GeneralException generalEx = (GeneralException)ex;
				assertThat(generalEx.getErrorReason().getCode()).isEqualTo(
	...
	void cancelPayment_Success() {
		...
		// When
		String result = paymentService.cancelPayment(cancelRequest, authentication);
	...
	void cancelPayment_ApiCallFailed() {
		...
		// When & Then
		assertThatThrownBy(() -> paymentService.cancelPayment(cancelRequest, authentication))
			.isInstanceOf(GeneralException.class)
			.satisfies(ex -> {
				GeneralException generalEx = (GeneralException)ex;
				assertThat(generalEx.getErrorReason().getCode()).isEqualTo(
